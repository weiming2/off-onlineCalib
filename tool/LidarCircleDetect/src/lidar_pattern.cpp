...
void LidarDetector::STLidarDetection(std::string pcds_dir) {
  ...
  velocloud = cloud;
  pcl::io::savePCDFileBinary("velocloud.pcd", *velocloud);

  pcl::PassThrough<pcl::PointXYZ> pass1;
  pass1.setInputCloud(velocloud);
  pass1.setFilterFieldName("x");
  // pass1.setFilterLimits(1, 3);
  ...
  plane_segmentation.setDistanceThreshold(0.05); //0.05
  plane_segmentation.setMethodType(pcl::SAC_RANSAC);
  plane_segmentation.setAxis(axis);
  plane_segmentation.setEpsAngle(0.3);
  ...
  for (int i = 0; i + 160 < pro_map.size(); i += 2) 
  {
    for (int j = 0; j + 160 < pro_map[0].size(); j += 2) 
    {
      int cnt = 0;
      for (int ii = i; ii - i <= 160; ii += 2) 
      {
        for (int jj = j; jj - j <= 160; jj += 2) 
        {
          if (pro_map[ii][jj])
            cnt++;
        }
  ...
  pcl::PointXYZ center;
  center.y = float(max_j) / 200 + min_pt_x + 0.17;
  center.z = float(max_i) / 200 + min_pt_y + 0.27;
  std::cout << center.y << ',' << center.z << std::endl;
  initial_centers->push_back(center);
  center.y = float(max_j) / 200 + min_pt_x + 0.17+ 0.56;
  center.z = float(max_i) / 200 + min_pt_y + 0.27;
  std::cout << center.y << ',' << center.z << std::endl;
  initial_centers->push_back(center);
  center.y = float(max_j) / 200 + min_pt_x + 0.17;
  center.z = float(max_i) / 200 + min_pt_y + 0.27+ 0.32;
  std::cout << center.y << ',' << center.z << std::endl;
  initial_centers->push_back(center);
  center.y = float(max_j) / 200 + min_pt_x + 0.17+ 0.56;
  center.z = float(max_i) / 200 + min_pt_y + 0.27+ 0.32;
