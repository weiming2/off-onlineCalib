...
public:
  ...
  void set_input_new(const std::vector<std::string> images_name,
                 const std::vector<cv::Mat> &vec_mat_,
                 const cv::Size &chessboard_size_,
                 const std::vector<std::vector<std::string>> &lidar_3d_pts);

	CANNYPARAM	m_cannnFactor;
  std::vector<std::vector<MatchPoint>>	m_codeinp;
...
private:
  ...
  std::vector<std::vector<cv::Point2f>> _boards_pts;     // 标定板坐标系2维坐标（x,y）
  std::vector<std::vector<cv::Point3f>> _boards_pts_3d;  // 标定板坐标系三维坐标（x,y ,0）
  std::vector<std::vector<cv::Point3f>> _boards_pts_cir; // 标定板角点三维坐标（x,y ,0）
