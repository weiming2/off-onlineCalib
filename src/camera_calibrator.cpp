...
void CameraCalibrator::set_input_new(
    const std::vector<std::string> images_name,
    const std::vector<cv::Mat> &vec_mat_, const cv::Size &chessboard_size_,
    const std::vector<std::vector<std::string>> &lidar_3d_pts) {
  // lidar
  ...
  int i = 0;
  bool isfail = false;
  m_cannnFactor.lowParam = 150;
	m_cannnFactor.highParam = 300;
  for (const auto &img : vec_mat_)
   {
    // CHECK(1 == img.channels()) << "images must be gray";
    std::vector<cv::Point2f> corner_pts;
    ...
    if (img.empty()) {continue;}
		HoleExtract onhe(img, 40 ,m_cannnFactor,63, images_name[i]);

		if (onhe.doEdgeExtract())
		{
      //???corner_pts 求出角点坐标，利用我们的新方法 corner_pts
			m_codeinp.push_back(onhe.CodedPI);

      auto nfirst = images_name[i].find_last_of('/');
      std::string img_name = images_name[i].substr(nfirst+1);
      std::cout<< img_name <<" 图像处理成功 \n"<< std::endl;

      char str[100];
      sprintf(str,"%d",i-1);
      std::string img_path ="results/" + std::string(str) + "_dstCoded.bmp" ;
      std::cout<< "img_path " << img_path << std::endl;
      // cv::imshow("m_dstCoded",onhe.m_dstCoded);
      // cv::imwrite(img_path,onhe.m_dstCoded);   
      // cv::waitKey(2);

      cv::Mat img_copy = img.clone();
      available_imgs.push_back(img_copy);
      code_imgs.push_back(onhe.m_dstCoded);
      //直接导入的
      // lidar_3d_pts_.push_back(pts[i]);
      ...
      for (size_t i = 0; i < onhe.CodedPI.size(); i++) 
      {
      cv::Point2f imguv;
      imguv.x = onhe.CodedPI[i].imgx;
      imguv.y = onhe.CodedPI[i].imgy;
      cv::Point2f objxy0;
      objxy0.x = onhe.CodedPI[i].objx;
      objxy0.y = onhe.CodedPI[i].objy;
      cv::Point3f objxyz;
      objxyz.x = onhe.CodedPI[i].objx;
      objxyz.y = onhe.CodedPI[i].objy;
      objxyz.z = onhe.CodedPI[i].objz;

      corner_pts.push_back(imguv);
      vec_points.push_back(objxy0);
      vec_points_3d.push_back(objxyz);
      }

      //下面的代码是找到_boards_pts_3d，_boards_pts，替换一下
      // this->make_board_points(chessboard_size_);
      //存储编码点
      ...
      _boards_pts_3d.push_back(vec_points_3d);
		...
		{
			isfail = true;
			// guidetet.Format(_T("第%d张图像处理失败"), i + 1);
			// pMain->UpdateGuideText(guidetet);
      i++;
      continue;
...
void CameraCalibrator::get_result(cv::Mat &camera_matrix, cv::Mat &k,
  ...
  std::cout << "camera_matrix is " << camera_matrix << std::endl;
  ...
  if (true) // Show the optimized projection effect
  {
    for (size_t i = 0; i < lidar_point_pairs.size(); i++) {
      ...
      std::string save_name = "new_refine" + std::to_string(i) + ".png";
      cv::imwrite(save_name, undistort_img);
...
void CameraCalibrator::make_board_points(const cv::Size &chessboard_size_) {
  ...
  vec_points_cir.emplace_back(2.0f, -3.12f, 0.0f);  // sim
